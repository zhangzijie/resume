    ...
    <div class="content">
        <div class="row">
            ...
            <div class="col-sm-9 col-md-10">
                <h3 class="sub-header">招聘单位管理</h3>
                <div class="table-responsive">
                    <form class="form-inline" id="search-form" action="searchCompany" method="post">
                    ...
                    <table class="table table-striped">
                        ...
                        <tbody>
                        <c:if test="${null == companyList[0]}">
                            <tr>
                                <td>没有用户！</td>
                            </tr>
                        </c:if>
                        <c:if test="${not empty companyList && not empty page}">
                            <c:forEach var="companyitem" items="${companyList}" varStatus="status" begin="${page.firstResult}" end="${page.lastResult}">
                                <tr>
                                    <td>${companyitem.username}</td>
                                    <td>${companyitem.companyname}</td>
                                    <td>${companyitem.linkman}</td>
                                    <td><a href="editCompany.action?id=${companyitem.id}"><span class="glyphicon glyphicon-pencil" aria-hidden="true" style="margin-left: 5px;"></span></a></td>
                                    <td><a href="#" id="deleteCompany.action?id=${companyitem.id}" class="deleteButton" data-toggle="modal" data-target="#deleteDialog"><span class="glyphicon glyphicon-remove" aria-hidden="true" style="margin-left: 5px;"></span></a></td>
                                </tr>
                            </c:forEach>
                        </c:if>
                        </tbody>
                        ...
                        <nav>
                          <ul class="pagination">
                            ...
                            <c:forEach var="s" begin="0" end="${page.maxPage-1}" varStatus="status">
                               ...
                               <li class="active"><a href="listCompany.action?pagenum=${status.index+1}">${status.index+1}</a></li>
                               </c:if>
                               <c:if test="${status.index+1 ne page.pageNum}">
                               <li><a href="listCompany.action?pagenum=${status.index+1}">${status.index+1}</a></li>
                               </c:if>
                            </c:forEach>
                            <li>
